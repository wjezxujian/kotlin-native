...
call cinterop -def "%DIR%\src\c_interop\sdl.def" -compilerOpts "%CFLAGS%" -target "%TARGET%" -o sdl || exit /b
rem Windows build requires Windows Resource Compiler in paths.
call windres "%DIR%\Tetris.rc" -O coff -o "%DIR%\Tetris.res" || exit /b
call konanc -target "%TARGET%" "%DIR%\src\kotlin-native\Tetris.kt" -library sdl -linkerOpts "%LFLAGS%" -opt -o Tetris || exit /b

copy Tetris.kexe Tetris.exe
copy %MINGW%\bin\SDL2.dll SDL2.dll
copy src\resources\tetris_all.bmp tetris_all.bmp
