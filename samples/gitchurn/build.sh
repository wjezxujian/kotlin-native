...
cinterop -compilerOpts $CFLAGS -def $DIR/src/c_interop/libgit2.def -target $TARGET \
         -o $DIR/build/c_interop/libgit2.kt.bc || exit 1

konanc -target $TARGET $DIR/src/kotlin-native -library $DIR/build/c_interop/libgit2.kt.bc -linkerOpts "$LINKER_ARGS" \
